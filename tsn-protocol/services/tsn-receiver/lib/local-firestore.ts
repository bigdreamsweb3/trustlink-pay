...
class Ref {
  ...
  async get() { const db = await read(); return new Snap(this.id, db[this.collection]?.[this.id] ?? null); }
  async set(value: Row) { return queue(async () => { const db = await read(); (db[this.collection] ??= {})[this.id] = value; await write(db); }); }
  async delete() { return queue(async () => { const db = await read(); if (db[this.collection]) delete db[this.collection][this.id]; await write(db); }); }
  collection(name: string) { return new LocalCollection(`${this.collection}/${this.id}/${name}`); }
}
