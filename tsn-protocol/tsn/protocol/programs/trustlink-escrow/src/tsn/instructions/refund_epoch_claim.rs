...
pub fn refund_epoch_claim(ctx: Context<RefundEpochClaim>, slot: [u8; 32], lease_version: u64, commitment_digest: [u8; 32], refund_nullifier: [u8; 32], refund_amount: u64, expires_at_ts: i64, permit_signature: [u8; 64]) -> Result<()> {
    ...
    require!(ctx.accounts.private_settlement_config.enabled && now <= expires_at_ts && now <= ctx.accounts.settlement_dna.authorization_expiry_ts && expires_at_ts <= ctx.accounts.settlement_dna.authorization_expiry_ts && refund_amount > 0 && slot != [0;32], TsnError::PermitExpired);
    ...
    let refund_commitment = hashv(&[b"TSN_REFUND_COMMITMENT_V1", treasury_key.as_ref(), &slot, &commitment_digest, &refund_nullifier, ctx.accounts.sender_wallet.key().as_ref(), ctx.accounts.token_mint.key().as_ref(), &refund_amount.to_le_bytes(), &expires_at_ts.to_le_bytes()]).to_bytes();
