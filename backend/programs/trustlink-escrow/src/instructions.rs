...
pub fn migrate_legacy_config(
...

    {
        ...
        require!(
            data.len() >= 8 + 32 && data.len() < EscrowConfig::SPACE,
            TrustLinkEscrowError::InvalidConfigAuthority
        );
        require!(
            data[0..8] == EscrowConfig::discriminator(),
            TrustLinkEscrowError::InvalidConfigAuthority
        );

        let legacy_claim_verifier = Pubkey::new_from_array(
            data[8..40]
                .try_into()
                .map_err(|_| error!(TrustLinkEscrowError::InvalidConfigAuthority))?,
        );
        require_keys_eq!(
            ctx.accounts.authority.key(),
            legacy_claim_verifier,
            TrustLinkEscrowError::InvalidConfigAuthority
        );
    }

    let rent = Rent::get()?;
    let required_lamports = rent.minimum_balance(EscrowConfig::SPACE);
    if config_info.lamports() < required_lamports {
        invoke(
            &system_instruction::transfer(
                &ctx.accounts.authority.key(),
                &config_info.key(),
                required_lamports - config_info.lamports(),
            ),
            &[
    ...
    config_info.realloc(EscrowConfig::SPACE, false)?;

    let mut data = config_info.try_borrow_mut_data()?;
    data.fill(0);
    data[0..8].copy_from_slice(&EscrowConfig::discriminator());
    data[8..40].copy_from_slice(new_claim_verifier.as_ref());
...
pub fn claim_and_bind_first_wallet(
    ...
    require!(
        payment.payment_mode.is_secure(),
        TrustLinkEscrowError::InvalidPaymentMode
    );
    require!(
        payment.status.is_receiver_claimable(),
        TrustLinkEscrowError::PaymentNotPending
    );
    ...
    release_to_destination(
        ...
        ctx.accounts
            .requested_settlement_token_account
            .to_account_info(),
...
pub fn claim_invite_and_bind_first_wallet(
    ...
    let payment = &ctx.accounts.payment_account;
    require!(
        payment.payment_mode.is_invite(),
        TrustLinkEscrowError::InvalidPaymentMode
    );
    require!(
        payment.status.is_receiver_claimable(),
...
pub fn mark_expired(ctx: Context<MarkExpired>, _payment_id: [u8; 32]) -> Result<()> {
    let now = Clock::get()?.unix_timestamp;
    let payment = &mut ctx.accounts.payment_account;
    require!(
        payment.status.is_locked(),
        TrustLinkEscrowError::PaymentNotPending
    );
    require!(
        payment.expiry_ts < now,
        TrustLinkEscrowError::PaymentNotExpired
    );
...
pub fn refund_expired_payment(
    ...
    let payment = &ctx.accounts.payment_account;
    require!(
        payment.status.is_expired(),
        TrustLinkEscrowError::PaymentNotExpiredState
    );
    require_keys_eq!(
...
pub fn claim_refund(_ctx: Context<ClaimRefund>, _payment_id: [u8; 32]) -> Result<()> {
    Err(TrustLinkEscrowError::LegacyDirectRefundDisabled.into())
}
...
pub fn expire_payment_to_pool(
    _ctx: Context<ExpirePaymentToPool>,
    _payment_id: [u8; 32],
) -> Result<()> {
