...
export function IdentityTree({
  ...
  const tinValue = tin && !missingTinName && displayName.trim()
    ? `${tin} · ${displayName}`
    : tin;
  const nodes: IdentityNode[] = [
    tinValue
      ...
      : hideMissingNodes
        ? null
        : { label: "Transfer Identity Number", value: "Not linked", tone: "muted" },
    tinsIdentityPublicKey
      ? { label: "TINS identity PDA", value: shortenAddress(tinsIdentityPublicKey) }
      : hideMissingNodes
        ? null
        : { label: "TINS identity PDA", value: "Not available", tone: "muted" },
    handle ? { label: "TrustLink handle", value: `@${handle}` } : null,
    phoneNumber ? { label: "WhatsApp identity", value: phoneNumber } : null,
    walletLabel ? { label: "Settlement wallet", value: walletLabel } : null,
  ].filter((node): node is IdentityNode => Boolean(node));

  return (
    <div className={`rounded-[18px] border border-[var(--field-border)] bg-[var(--field)] ${compact ? "px-3 py-3" : "px-4 py-4"}`}>
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <div className="text-[0.64rem] text-[var(--text-faint)]">
            {nameSourceLabel ?? "One identity root mapped across TrustLink and TINS"}
          </div>
        </div>
      </div>
      ...
      {missingTinName ? (
        <div className="mt-3 rounded-[14px] border border-[#f3c96b]/25 bg-[#f3c96b]/10 px-3 py-2 text-[0.72rem] leading-relaxed text-[#f3c96b]">
          No identity name found on this transfer identity number.
        </div>
      ) : null}
      ...
      <div className="relative mt-4 space-y-2 pl-4">
        ...
        {nodes.map((node) => (
          <div key={`${node.label}:${node.value}`} className="relative flex items-center gap-3">
