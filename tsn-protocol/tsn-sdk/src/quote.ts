...
export function quoteTransferFeeUiAmount(input: TsnTransferFeeQuoteInput) {
  if (
    input.estimatedNetworkFeeLamports <= 0 ||
    !input.solUsd ||
    !input.tokenUsd ||
    input.tokenUsd <= 0 ||
  ...
  const marginUsd =
    input.maxMarginUsd > 0 ? Math.min(uncappedMarginUsd, input.maxMarginUsd) : uncappedMarginUsd;
