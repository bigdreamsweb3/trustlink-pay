...
function makeCodewords(value: string) {
  ...
  if (bytes.length > DATA_CODEWORDS - 2) {
    throw new Error("QR value is too long for the TrustLink auth QR code.");
  }
...
function placeData(baseMatrix: Cell[][], reserved: boolean[][], codewords: number[], mask: number) {
...
function createQrMatrix(value: string) {
  ...
  let best = placeData(baseMatrix, reserved, codewords, 0);
  let bestMask = 0;
  let bestPenalty = penalty(best);

  for (let mask = 1; mask < 8; mask += 1) {
    const candidate = placeData(baseMatrix, reserved, codewords, mask);
