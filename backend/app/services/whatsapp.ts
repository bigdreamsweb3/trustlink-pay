...
async function sendWhatsAppMessage(phoneNumber: string, payload: Record<string, unknown>) {
...
async function sendWhatsAppTextMessage(phoneNumber: string, body: string) {
...
async function sendWhatsAppTemplateMessage(phoneNumber: string, template: WhatsAppTemplatePayload) {
  return sendWhatsAppMessage(phoneNumber, {
    messaging_product: "whatsapp",
    to: phoneNumber.replace(/^\+/, ""),
    type: "template",
...
export async function sendPaymentNotification(params: {
  ...
  if (env.WHATSAPP_USE_TEMPLATES && env.WHATSAPP_PAYMENT_TEMPLATE_NAME) {
    return sendWhatsAppTemplateMessage(params.phoneNumber, {
...
export async function sendOtp(phoneNumber: string, otp: string) {
  if (env.WHATSAPP_USE_TEMPLATES && env.WHATSAPP_OTP_TEMPLATE_NAME) {
    return sendWhatsAppTemplateMessage(phoneNumber, {
