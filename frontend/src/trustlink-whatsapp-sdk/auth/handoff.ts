...
// Shorter URL intended for QR codes (keeps the payload small enough for our QR renderer).
export function buildTrustLinkWhatsAppQrUrl(params: { phoneNumber: string; sessionCode: string }) {
  const formattedNumber = params.phoneNumber.replace(/[^0-9]/g, "");
  const encodedMessage = encodeURIComponent(buildTrustLinkWhatsAppHandoffMessage(params.sessionCode));
