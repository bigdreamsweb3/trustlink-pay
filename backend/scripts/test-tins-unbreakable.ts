...
async function main() {
  ...
  console.log(
    "✓ Identity Seed generated correctly:",
    identitySeed.toString("hex"),
  );
  ...
  console.log(
    "✓ Phone successfully encrypted. Blob length:",
  ...
  assert(
    createParams.readUInt8(0) === 4,
    "CreateTin instruction tag must be 4",
  );
  assert(
    createParams.readUInt32LE(1) === displayName.length,
    "CreateTin display name length mismatch",
  );
  assert(
    createParams.readUInt32LE(1 + 4 + displayName.length) ===
      encryptedBlob.length,
