...
import { signSolanaMessage, signSolanaTransaction } from "@/src/lib/wallet";
...
export function SendExperience() {
  ...
  const previousReviewFocusRef = useRef<HTMLElement | null>(null);
  ...
  async function lookupResolvedTin(tin: string) {
    ...
    try {
      enrichment = await apiPost<RecipientLookupResult>(
        "/api/recipient/lookup",
        { tin },
        undefined,
        { cache: "default", ttlMs: 60_000 },
      ...
      // TrustLink indexing is optional for a valid on-chain TIN.
    }
    const trustLinkLinked = enrichment?.status === "registered";
    ...
    const displayName =
      onchain.legalName ||
      identityName ||
      trustLinkRecipient?.displayName ||
      `TIN ${tin}`;
    const recipient: RecipientLookupResult = {
      status: "tins_resolved",
      verified: true,
      recipient: {
        displayName,
        handle: trustLinkRecipient?.handle ?? null,
        phoneNumber: normalizedPhone.startsWith("tin:") ? "" : normalizedPhone,
        source: "tins",
        tin,
        whatsappProfileName: trustLinkRecipient?.whatsappProfileName ?? null,
        identityName,
        legalName: onchain.legalName,
        legalNameStatus: onchain.legalName ? "verified" : "not_available",
        settlementWallet: onchain.authority,
        registryAddress: onchain.registry,
        trustLinkLinked,
        socialIdentities: onchain.socialIdentities.map((identity) => ({
          type: identity.type,
      ...
      ...(!identityName
        ? { warning: "No public identity name is linked to this TIN." }
        : {}),
    };
    let verification: WhatsAppNumberVerificationResult = {
      phoneNumber: normalizedPhone,
      exists: true,
    ...
    if (!normalizedPhone.startsWith("tin:")) {
      try {
        verification = await apiPost<WhatsAppNumberVerificationResult>(
          "/api/whatsapp/verify-number",
          { phoneNumber: normalizedPhone },
          undefined,
          { cache: "default", ttlMs: 5 * 60_000 },
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (observedStatus) {
          setSendSuccess((current) => {
            if (!current || current.paymentId !== paymentIdForPolling)
              return current;
            return {
              ...current,
              tsn: toPaymentTsnState(
                observedStatus,
                current.tsn?.destinationWallet ?? null,
              ),
              status:
    ...
    accessToken,
    sendSuccess,
    sendSuccessPaymentId,
    shouldPollSendSuccessReceipt,
  ]);

  useEffect(() => {
    if (!confirmOpen) return;

    previousReviewFocusRef.current =
    ...
    return () => {
      ...
      previousReviewFocusRef.current?.focus();
    };
  }, [confirmOpen]);
  ...
  return (
    ...
    >
                      ...
                      recipientPreview.status === "tins_resolved" ? (
                        <div className="mt-3 space-y-2 rounded-[14px] border border-[var(--accent-border)] bg-[var(--accent-soft)] px-3 py-3">
                          ...
                          <div className="text-[0.66rem] leading-relaxed text-text-faint">
                            {recipientPreview.recipient.trustLinkLinked
                              ? `TrustLink profile: @${recipientPreview.recipient.handle}`
      ...
      {confirmOpen && recipientPreview?.verified && selectedToken ? (
        ...
        >
          <div
            className="tl-modal w-full rounded-t-[28px] md:rounded-[28px] md:max-w-[520px] flex flex-col max-h-[calc(100dvh-32px)] md:max-h-[calc(100dvh-32px)] overflow-hidden"
            onClick={(e) => e.stopPropagation()}
            ref={reviewDialogRef}
            role="dialog"
            aria-modal="true"
          ...
          >
            ...
            <div className="shrink-0 px-4 md:px-6 pt-4 md:pt-6 pb-2 md:pb-3 flex items-start justify-between gap-3">
              ...
              <button
                type="button"
                onClick={() => setConfirmOpen(false)}
                className="shrink-0 text-text-soft hover:text-text transition-colors"
            ...
            <div className="flex-1 min-h-0 overflow-y-auto px-4 md:px-6 py-4 md:py-5 space-y-3 md:space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="text-right">
                    ...
                    recipientPreview.recipient.whatsappProfileName !==
                      recipientPreview.recipient.displayName ? (
                      <div className="mt-0.5 text-[0.72rem] text-text-soft">
                        {recipientPreview.recipient.whatsappProfileName}
                      </div>
                    ) : null}
                  </div>
