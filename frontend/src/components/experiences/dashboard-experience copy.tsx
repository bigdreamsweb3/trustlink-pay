...
export function DashboardExperience() {
  ...
  const stats = useMemo(
    () => [
      {
        label: "Pending",
        value: pendingPayments.length.toString().padStart(2, "0"),
      },
    ],
  ...
  return (
    ...
    >
      <section className="space-y-5">
        {/* ─── BALANCE HERO CARD ─── */}
        <div className="tl-scanline relative overflow-hidden rounded-[30px] text-text border border-accent-border bg-accent-gradient p-5 shadow-softbox">
          ...
          <div className="relative z-10 flex items-start justify-between gap-4">
            ...
            >
              <div className="flex items-center gap-1.5">
                <span className="rounded-[6px] border border-white/6 bg-white/4 px-1.5 py-0.5 text-[0.66rem] font-semibold text-text/50">
                  {countryCode}
                </span>
                <span className="text-[0.84rem] font-bold tracking-wide text-text/78">
                  {localNumber}
                </span>
          ...
          <div className="relative z-10 mt-6 flex items-end justify-between gap-3">
            ...
            <div className="flex flex-col items-end gap-2 justify-end">
              {stats.map((stat) => (
                <div
                  key={stat.label}
                  className="w-fit flex items-center gap-1.5 rounded-[14px] border border-white/5 bg-white/3 px-3 py-2"
                >
        ...
        {loading ? (
          ...
          <Link
            href="/app/claim"
            className="tl-field group flex items-center justify-between rounded-[22px] px-4 py-4 transition-colors hover:bg-[var(--surface-soft)] cursor-pointer active:scale-[0.99]"
          >
            <div className="min-w-0 flex-1">
              <div className="text-[0.62rem] uppercase tracking-[0.2em] text-accent/68">
                Pending claims
              </div>
        ...
        <div>
          ...
          {!loading && paymentHistory.length > 0 ? (
            ...
            >
              <span className="text-[0.84rem] font-medium text-[var(--text)]">
                View all activity
              </span>
              <ChevronRight className="h-4 w-4 text-[var(--text-faint)] transition-transform group-hover:translate-x-0.5" />
            </Link>
          ...
          >
            <div className="mb-5 flex items-start justify-between gap-4">
              <div className="min-w-0 flex-1">
                ...
                <p className="tl-text-muted mt-1 text-[0.82rem] leading-relaxed">
              ...
              <button
                type="button"
                onClick={() => setBalanceInfoOpen(false)}
                className="tl-button-secondary shrink-0 rounded-full px-3.5 py-2 text-xs font-medium cursor-pointer transition-colors hover:opacity-90 active:scale-[0.97]"
              >
                Close
              </button>
            </div>
            <div className="space-y-2.5">
              <div className="tl-field rounded-[18px] px-4 py-3.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[0.92rem] font-semibold text-[var(--text)]">
                    {balanceVisible
                      ? formatPaymentUsd(combinedVisibleBalanceUsd)
                      : "****"}
                  </span>
              ...
              <div className="tl-field rounded-[18px] px-4 py-3.5">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <span className="block text-[0.84rem] font-semibold text-[var(--text)]">
                      {balanceVisible
              ...
              <div className="tl-field rounded-[18px] px-4 py-3.5">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <span className="block text-[0.68rem] text-[var(--text-soft)]">
                      {pendingBalanceSummary.claimableCount}{" "}
                      {pendingBalanceSummary.claimableCount === 1
                        ? "payment"
                        : "payments"}
                    </span>
                  </div>
