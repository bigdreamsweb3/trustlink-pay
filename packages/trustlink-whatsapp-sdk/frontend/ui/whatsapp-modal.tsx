...
export function WhatsAppModal({
  ...
  const [handoffStatus, setHandoffStatusState] = useState<
    "idle" | "opening" | "waiting" | "fallback"
  >("idle");
  const [copiedField, setCopiedField] = useState<"phone" | "message" | null>(
    null,
  );
  ...
  const setHandoffStatus = useCallback(
    (status: "idle" | "opening" | "waiting" | "fallback") => {
  ...
  const handleOpenWhatsApp = useCallback(() => {
    ...
    const nativeWhatsAppUrl = buildTrustLinkWhatsAppNativeUrl({
      phoneNumber,
      sessionCode,
    });
          ...
          >
            ...
            >
              {(handoffStatus === "opening" || handoffStatus === "waiting") && (
                <div
                  className="sticky left-0 right-0 top-0 z-20 h-[2px] overflow-hidden"
                  style={{ background: "var(--surface-soft)" }}
                >
                  <motion.div
                    initial={{ width: "0%" }}
                    animate={{
                      width: handoffStatus === "waiting" ? "84%" : "45%",
                    }}
                    transition={{
                      duration: handoffStatus === "opening" ? 2 : 0.4,
                      ease: "easeOut",
                    }}
                    className="h-full"
