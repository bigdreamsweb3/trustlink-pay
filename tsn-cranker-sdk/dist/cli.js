...
async function handleCommand() {
    ...
    if (command === "register-cranker") {
        console.log(await tsnRegisterCrankerOnChain({
    ...
    if (command === "set-funding-policy") {
        console.log(await tsnSetCrankerFundingPolicyOnChain({
            operator: operatorKeypair(),
    ...
    if (command === "init-vault") {
        ...
        console.log(await tsnInitializeCrankerVaultOnChain({
            payer: operator,
    ...
    if (command === "fund-cranker") {
        const operator = operatorKeypair();
        console.log(await tsnFundCrankerOnChain({
    ...
    if (command === "withdraw-cranker") {
        const operator = operatorKeypair();
        console.log(await tsnWithdrawCrankerFundsOnChain({
            funder: loadKeypair(process.argv[4]),
            operator: operator.publicKey,
            tokenMint: new PublicKey(process.argv[3]),
            funderTokenAccount: new PublicKey(process.argv[5]),
    ...
    if (command === "settle-epoch") {
        console.log(await tsnSettleEpochOnChain({
            authority: authorityKeypair(),
    ...
    if (command === "race-epoch") {
        const operator = operatorKeypair();
        console.log(await tsnProcessBatchReimbursementOnChain({
            operator,
