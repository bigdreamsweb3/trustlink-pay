...
const DEFAULT_ALLOWED_TOKENS: TsnAllowedToken[] = [
  {
...
function normalizeToken(input: unknown): TsnAllowedToken | null {
...
export function tsnGetAllowedSplTokens(
  ...
  try {
    ...
    if (!Array.isArray(parsed)) return DEFAULT_ALLOWED_TOKENS;
    const normalized = parsed
      .map((entry) => normalizeToken(entry))
      .filter((entry): entry is TsnAllowedToken => entry !== null);
...
export function tsnResolveSplTokenInput(
  tokenInput: string,
  env: Record<string, string | undefined>,
): TsnAllowedToken {
