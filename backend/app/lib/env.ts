...
const envSchema = z
  .object({
    ...
    TRUSTLINK_CLAIM_BASE_URL: z.string().url().default("https://trustlink.app/claim"),
    ...
    OTP_RATE_LIMIT_MAX_REQUESTS_PER_IP: z.coerce.number().int().positive().default(5),
  ...
  .superRefine((value, context) => {
    if (!value.WHATSAPP_MOCK_MODE && !value.WHATSAPP_PHONE_ID) {
      context.addIssue({
        ...
        message: "WHATSAPP_PHONE_ID is required when WHATSAPP_MOCK_MODE is false"
      });
    }
  });
...
function readRawEnv() {
  return {
    ...
    OTP_RATE_LIMIT_MAX_REQUESTS_PER_IP: process.env.OTP_RATE_LIMIT_MAX_REQUESTS_PER_IP,
