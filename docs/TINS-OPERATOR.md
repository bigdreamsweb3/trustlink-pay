...
Build the TINS program from the `tins-registrar/program` directory:
...
Deployment must be performed with the real TINS program keypair. The keypair public key must equal `TinseNnU588NkmRZBe4ADJbxqrqQma92678UFP6VuwT`.
...
Initialize the global state before creating TINs. The global state account uses seed `global-state`.
...
| `identity_pubkey` | Wallet-derived TINS account |
...
2. Confirm the global state account exists.
