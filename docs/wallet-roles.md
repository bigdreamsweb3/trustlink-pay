...
Before continuing with treasury fee routing:

1. create `trustlink-treasury.json`
2. add its public key to `TRUSTLINK_TREASURY_OWNER`
3. redeploy the upgraded escrow program with treasury fee-splitting support
4. run `npm run escrow:init-config` again against the upgraded deployment
