...
export function evaluateSettlementEconomics(input) {
    ...
    if (!input.solUsd || !input.tokenUsd || input.tokenUsd <= 0) {
        ...
        const likelihood = net <= 0 ? "economically_non_claimable" : net < input.paymentAmountUi * 0.15 ? "risky_claim_amount" : "likely_claimable";
        return {
            likelihood,
