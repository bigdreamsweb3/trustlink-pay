...
export function getTsnPaymentVaultPda(paymentIntentId: bigint): PublicKey {
  return PublicKey.findProgramAddressSync(
...
export async function tsnSubmitSenderSignedSettlementTransaction(params: {
  ...
  if (!tx.feePayer?.equals(params.operator.publicKey)) {
    throw new Error(
      `Sender-signed settlement fee payer mismatch. Expected cranker ${params.operator.publicKey.toBase58()}, got ${tx.feePayer?.toBase58() ?? "missing"}.`,
    );
  }
...
export async function tsnClaimVaultSettlementOnChain(params: {
  ...
  const signature = await sendAndConfirmTransaction(
    connection,
    new Transaction({ feePayer: params.operator.publicKey }).add(ix),
    [params.operator],
  ...
  return { mode: "devnet" as const, signature, paymentVault: paymentVault.toBase58() };
}
