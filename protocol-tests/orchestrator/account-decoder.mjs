...
export function decodeFundingAccounts({ reserve, root, nonce, claim }) {
  return {
    reserve: reserve ? { rawHash: hashAccount(reserve.data), actualAssets: reserve.data.readBigUInt64LE(108).toString(), pendingLiabilities: reserve.data.readBigUInt64LE(116).toString() } : null,
