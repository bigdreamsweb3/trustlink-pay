...
export function assertVerifiedTsnProgramId(programId) {
    const resolved = typeof programId === "string" ? programId : programId.toBase58();
    if (resolved !== VERIFIED_TSN_PROGRAM_ID) {
        throw new Error(`Unverified TSN program id ${resolved}. This SDK only supports verified ${VERIFIED_TSN_PROGRAM_LABEL} ${VERIFIED_TSN_PROGRAM_ID} on ${VERIFIED_TSN_CLUSTER}. Update the TSN package to use a newer verified program.`);
    }
    return resolved;
}
