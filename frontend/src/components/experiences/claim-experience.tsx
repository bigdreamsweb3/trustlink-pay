...
export function ClaimExperience({ paymentId }: { paymentId: string }) {
  ...
  async function handleClaim() { if (!accessToken || !activeWalletAddress || !payment) { setError("Connect wallet first."); return; } setClaimBusy(true); setError(null); try { const bundle = getOrCreatePrivacyKeyBundle(); if (!payment.payment.phone_identity_pubkey || !payment.payment.payment_receiver_pubkey || !payment.payment.ephemeral_pubkey) throw new Error("Privacy routing data missing"); const phoneIdentityPublicKey = payment.payment.phone_identity_pubkey; const paymentReceiverPublicKey = payment.payment.payment_receiver_pubkey; const ephemeralPublicKey = payment.payment.ephemeral_pubkey; if (bundle.phoneIdentityPublicKey !== payment.payment.phone_identity_pubkey) throw new Error("Wrong device privacy keys"); const derivedPaymentReceiver = await derivePaymentReceiverPublicKey({ privacyViewPrivateKey: bundle.privacyViewPrivateKey, privacySpendPublicKey: bundle.privacySpendPublicKey, ephemeralPublicKey }); if (derivedPaymentReceiver !== paymentReceiverPublicKey) throw new Error("Routing verification failed"); const privacySpendSignature = signClaimProof({ privacySpendPrivateKey: bundle.privacySpendPrivateKey, paymentId, phoneIdentityPublicKey, paymentReceiverPublicKey, ephemeralPublicKey, settlementWalletPublicKey: activeWalletAddress }); const prepared = await apiPost<{ serializedTransaction: string | null; rpcUrl: string | null; programId: string | null; preview: { escrowAccount: string; escrowVaultAddress: string; settlementWallet: string; settlementTokenAccount: string; paymentReceiverPublicKey: string; amount: number; tokenMintAddress: string } | null; claimFeeAmount: string | null; requiresClientSignature: boolean }>("/api/payment/accept", { paymentId, pin, walletAddress: activeWalletAddress, derivedPaymentReceiverPublicKey: derivedPaymentReceiver, privacySpendSignature }, accessToken); if (!prepared.serializedTransaction || !prepared.rpcUrl || !prepared.programId || !prepared.preview || !session) throw new Error("Transaction could not be prepared"); const expectedProgramId = prepared.programId; const receiverPrivateKey = await deriveReceiverPrivateKey({ privacyViewPrivateKey: bundle.privacyViewPrivateKey, privacySpendPrivateKey: bundle.privacySpendPrivateKey, ephemeralPublicKey }); const blockchainSignature = await signAndSendSerializedSolanaTransaction({ walletId: session.walletId, rpcUrl: prepared.rpcUrl, serializedTransaction: prepared.serializedTransaction, partialSignerSecretKeys: [receiverPrivateKey], inspectTransaction: async (transaction) => { await assertClaimTransactionIntegrity({ rpcUrl: prepared.rpcUrl!, transaction, paymentId, escrowAccount: payment.payment.escrow_account ?? prepared.preview!.escrowAccount, escrowVaultAddress: payment.payment.escrow_vault_address ?? prepared.preview!.escrowVaultAddress, settlementWallet: activeWalletAddress, settlementTokenAccount: prepared.preview!.settlementTokenAccount, phoneIdentityPublicKey, paymentReceiverPublicKey: derivedPaymentReceiver, tokenMintAddress: payment.payment.token_mint_address ?? prepared.preview!.tokenMintAddress, expectedProgramId, expectedAmountUi: grossAmount }); } }); const result = await apiPost<{ referenceCode: string; walletAddress: string; claimFeeAmount: string | null; netAmount: number | null; tokenSymbol: string | null; blockchainSignature: string | null }>("/api/payment/accept", { paymentId, pin, walletAddress: activeWalletAddress, derivedPaymentReceiverPublicKey: derivedPaymentReceiver, privacySpendSignature, blockchainSignature }, accessToken); setStatus(`Reference ${result.referenceCode} received.`); setClaimSuccess({ ...result, claimFeeAmount: result.claimFeeAmount != null ? Number(result.claimFeeAmount) : null }); setPinModalOpen(false); showToast("Payment received."); } catch (e) { setError(e instanceof Error ? e.message : "Could not receive payment"); } finally { setClaimBusy(false); } }
