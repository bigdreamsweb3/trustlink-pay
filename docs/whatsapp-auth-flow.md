...
This document describes the WhatsApp-based authentication flow for TrustLink Pay, which replaces the traditional OTP system with a more secure and user-friendly session code mechanism.
...
const sessionCodeMatch = inboundText.match(
  /Verify\s+TLPay\s+Code:\s+(TL[A-Z0-9]{6})/i,
);
```
