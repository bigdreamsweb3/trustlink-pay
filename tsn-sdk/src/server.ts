...
createServer(async (request, response) => {
  try {
    ...
    if (request.method === "GET" && path === "/intents") {
      return send(response, 200, await mempool.listIntents({ status: url.searchParams.get("status") as any }));
    }
    ...
    if (request.method === "GET" && path === "/recovery-queue") {
      return send(response, 200, await mempool.listRecoveryQueue({ status: url.searchParams.get("status") as any }));
    }
