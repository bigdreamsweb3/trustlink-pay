...
async function fetchEscrowConfigFromChain(connection, programId) {
    const [motherEscrowPda] = PublicKey.findProgramAddressSync([new TextEncoder().encode("tsn_mother_escrow")], programId);
    const account = await connection.getAccountInfo(motherEscrowPda, "confirmed");
    if (!account?.data) {
        throw new Error("TSN mother escrow not found on-chain. Run init-mother for this TSN program.");
    }
    const data = new Uint8Array(account.data);
    const minimumMotherEscrowLength = 8 + 32 + 32 + 32 + 8 + 8 + 2 + 2 + 2 + 8 + 8 + 1;
...
async function estimateNetworkFeeLamports(connection, senderWallet) {
...
export async function estimateTsnSendCostFromChain(params) {
    ...
    return {
        ...
        senderFeeAmountUi,
        senderFeeAmountUsd,
        totalTokenRequiredUi: Number((params.amountUi + senderFeeAmountUi).toFixed(6)),
        networkFeeSol,
        networkFeeUsd,
        debug: {
