...
const heroStats = [
  ...
  },
  {
    ...
    value: "TINS",
    note: "10-digit Transfer Identity Number",
  },
...
export function LandingPage() {
  ...
  return (
    <main className="app-shell overflow-hidden text-[14px] md:text-[15px]">
      ...
      <section
        id="tsn-protocol"
        className="mx-auto grid min-h-[calc(100dvh-7rem)] w-full max-w-[1180px] scroll-mt-14 items-center gap-10 px-2 pb-12 pt-2 lg:grid-cols-[0.98fr_1.02fr]"
      >
        <div className="relative z-10">
          ...
          <h1 className="tl-display mt-5 max-w-[760px]">
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-accent to-accent-deep">
              payments as familiar as mobile money.
            </span>
          </h1>
          <p className="tl-body-lg mt-5 pr-3 max-w-[680px]">
            ...
            payments. TrustLink Pay brings that same familiar identity layer to
            Solana stablecoins, while TSN routes settlement through temporary
            escrow and private claim flows instead of direct wallet-to-wallet
            transfers.
          </p>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              href="#how-it-works"
              className="tl-button-primary tl-btn inline-flex items-center gap-2 rounded-[14px] px-4 py-2.5"
            >
              See how it works <ArrowRight className="h-4 w-4" />
            </Link>
            <Link
              href="/app"
              className="tl-button-secondary tl-btn inline-flex items-center gap-2 rounded-[14px] px-4 py-2.5"
            >
              Open Dapp
            </Link>
            <a
        ...
        <div
          ...
          className="relative z-10 grid gap-3 w-full scroll-mt-14"
        >
          <TxProcessAnimator />
          ...
          <div className="grid grid-cols-2 gap-3">
            {heroStats.map((stat) => (
              <div key={stat.label} className="tl-field rounded-[14px] p-4">
                <p className="text-[0.65rem] font-bold uppercase tracking-[0.14em] text-text-faint">
                  {stat.label}
                </p>
                <strong className="mt-2 block text-xl font-black text-[var(--text)]">
                  {stat.value}
                </strong>
      ...
      <section
        id="identity-first"
        className="mx-auto grid w-full max-w-[1180px] scroll-mt-14 gap-8 px-2 py-14 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:px-0"
      >
      ...
      >
        <div className="min-w-0">
          ...
          <h2 className="tl-h2 mt-5">
            Traditional wallet transfers expose both parties directly during
          ...
          <p className="tl-body mt-5 max-w-[680px]">
            ...
            infrastructure. Settlement remains verifiable on-chain while
            abstracting complexity away from normal users.
          </p>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          {[
            ...
            <article
              key={idx}
              className="tl-panel rounded-[18px] p-5 flex items-center gap-4"
            >
              {/* @ts-ignore */}
              <Icon className="h-6 w-6 text-accent shrink-0" />
              <h3 className="text-[0.95rem] font-bold text-[var(--text)]">
                {String(title)}
              </h3>
            </article>
          ))}
      ...
      <section
        id="tsn"
        className="mx-auto w-full max-w-[1180px] scroll-mt-14 px-2 py-14 sm:px-6 lg:px-0"
      >
        ...
        <div className="mt-6 grid gap-8 lg:grid-cols-2">
          ...
          <div className="tl-panel rounded-[18px] p-6">
            <h3 className="text-[1.2rem] font-black text-[var(--text)] mb-4">
              Transparent Fees
            </h3>
            ...
            <div className="grid gap-2 border-t border-[var(--field-border)] pt-4">
              {transparentFeesRows.map(([label, note]) => (
                <div
                  key={label}
                  className="grid gap-2 border-b border-[var(--field-border)] pb-4 last:border-b-0 md:grid-cols-[1fr_1.5fr]"
                >
      ...
      >
        ...
        <div className="mt-6 grid gap-8 lg:grid-cols-2 items-center">
          <div>
            <h2 className="tl-h2">Programmable payment infrastructure.</h2>
            <p className="mt-4 text-[1rem] leading-8 text-[var(--text-soft)]">
      ...
      <section
        id="status"
        className="mx-auto w-full max-w-[1180px] scroll-mt-14 px-2 py-14 sm:px-6 lg:px-0 mb-12"
      >
        <div className="tl-panel rounded-[24px] p-8 md:p-12 text-center bg-gradient-to-b from-[var(--bg)] to-[var(--bg-soft)] border border-[var(--field-border)]">
          ...
          <p className="mt-8 text-[0.95rem] text-[var(--text-faint)] max-w-[600px] mx-auto">
            Additional settlement infrastructure continues evolving through TSN
            and TINS development.
            <br />
            <br />
            <strong>
              TrustLink Pay: Identity-first stablecoin settlement infrastructure
              on Solana.
            </strong>
          </p>
        </div>
