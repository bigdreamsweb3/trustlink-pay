...
describe("tsn_flow (milestone 4)", () => {
  ...
  before(async () => {
    ...
    await provider.connection.confirmTransaction(
      await provider.connection.requestAirdrop(crankerOperator.publicKey, 2 * anchor.web3.LAMPORTS_PER_SOL),
      "confirmed",
    );
  ...
  it("initializes base escrow config (if needed)", async () => {
    const existing = await provider.connection.getAccountInfo(configPda, "confirmed");
    if (existing) return;
  ...
  it("initializes TSN mother escrow + registers cranker", async () => {
    const existing = await provider.connection.getAccountInfo(motherEscrowPda, "confirmed");
    if (!existing) {
      await program.methods
    ...
    const existingCranker = await provider.connection.getAccountInfo(crankerPda, "confirmed");
    if (!existingCranker) {
      await program.methods
        .tsnRegisterCranker()
        .accounts({
    ...
    const motherEscrow = await (program.account as any).motherEscrow.fetch(motherEscrowPda);
  ...
  it("creates payment -> cranker submits intent -> earns and spends claim credit", async () => {
    ...
    const [intentPda] = anchor.web3.PublicKey.findProgramAddressSync(
      [Buffer.from("tsn_intent"), motherEscrowPda.toBuffer(), Buffer.from(intentId)],
      program.programId,
    );
