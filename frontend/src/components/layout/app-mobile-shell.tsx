...
const sidebarNavItems: Array<{
...
}> = [
  {
    ...
    href: "/app",
    label: "Home",
    icon: <HomeIcon size={18} className="text-current" />,
  },
  {
    key: "send",
    href: "/app/send",
  ...
  return (
    <main className="min-h-screen bg-bg">
        ...
        <aside className="tl-sidebar hidden md:sticky md:top-0 md:flex md:h-screen md:w-[240px] md:shrink-0 md:flex-col md:justify-between md:border-r md:border-[var(--field-border)] md:px-4 md:py-6">
          <div>
            <div className="flex items-center gap-3 px-2 mb-8">
              ...
              <div className="min-w-0">
                <div className="tl-text-muted text-[0.6rem] uppercase tracking-[0.22em] leading-none">
                  TrustLink
                </div>
            ...
            <nav aria-label="Sidebar navigation" className="space-y-1">
              {sidebarNavItems.map((item) => {
                ...
                return (
                  ...
                  >
                    {active ? (
                      <span className="absolute left-0 top-1/2 h-4 w-[3px] -translate-y-1/2 rounded-r-full bg-[var(--accent-deep)] dark:bg-[var(--accent)]" />
                    ) : null}
                    <span
                      className={`transition-colors ${active ? "text-[var(--accent-deep)] dark:text-[var(--accent)]" : "tl-text-soft group-hover:text-[var(--text-soft)]"}`}
                    >
                      {item.icon}
                    </span>
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block truncate text-[0.82rem] font-semibold leading-tight text-text">
                {user.displayName}
              </span>
              <span className="tl-text-muted block truncate text-[0.66rem] leading-tight mt-0.5">
                @{user.handle}
              </span>
            </span>
          </Link>
            ...
            <div className="flex items-center gap-3">
              {showBackButton ? (
                <button
                  type="button"
                  onClick={handleBack}
                  className="tl-text-soft inline-flex items-center gap-1.5 text-[0.82rem] font-medium transition hover:text-[var(--text)] cursor-pointer active:scale-[0.97]"
                >
                  <BackIcon className="h-4 w-4" />
                  <span>Back</span>
                </button>
              ) : null}
              <h1 className="text-[1rem] font-semibold tracking-[-0.02em] text-[var(--text)]">
                {title}
              </h1>
            </div>
            ...
            <div className="flex items-center gap-1.5 min-w-0 flex-1">
              {showBackButton ? (
                <button
                  type="button"
                  onClick={handleBack}
                  className="tl-text-soft inline-flex items-center gap-1 text-[0.74rem] font-medium transition hover:text-[var(--text)] cursor-pointer active:scale-[0.95]"
                  aria-label="Go back"
                >
              ...
              <TrustLinkMark />
              {!showBackButton ? (
                <div className="tl-text-muted text-[0.6rem] uppercase tracking-[0.22em] leading-none hidden">
                  TrustLink
                </div>
              ) : null}
            </div>
      ...
      <nav
        ...
        className="fixed bottom-3 left-1/2 z-40 grid max-h-17 w-[calc(100%-1rem)] max-w-99.5 -translate-x-1/2 grid-cols-5 items-start gap-0 rounded-3xl border border-dock-border bg-dock px-2 pb-[max(0.4rem,env(safe-area-inset-bottom))] pt-2 shadow-softbox backdrop-blur-2xl md:hidden"
      >
        {mobileNavItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`text-[0.58rem] font-medium tracking-[-0.01em] leading-tight ${active ? "text-[var(--text)]" : "tl-text-muted"}`}
              >
                {item.label}
              </span>
            </Link>
