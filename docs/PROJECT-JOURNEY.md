...
TrustLink Pay developed from a familiar payment experience toward a protocol design built around Transfer Identity Numbers, PRU routing, and TSN settlement. Phone and WhatsApp signals can help users assess recipients, but the TIN is the public payment identity. TSN exists because identity resolution alone does not prevent a simple public wallet-to-wallet payment graph.
...
- TSN provides the settlement workflow for payment intents, PRU routing, Cranker execution, proof records, and epoch-aware accounting.
